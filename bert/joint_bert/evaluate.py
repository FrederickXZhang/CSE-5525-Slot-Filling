...
def get_results(input_ids, input_mask, segment_ids, valid_positions, sequence_lengths, tags_arr, intents, tags_vectorizer, intents_label_encoder):
    ...
    with open("pred.outputs", 'w') as out:
        for tags in predicted_tags:
            out.write(str(" ".join(tags))+"\n")
            
    with open("real.outputs", 'w') as out:
        for tags in real_tags:
            out.write(str(" ".join(tags))+"\n")
    ...
    # sentence_correct = metrics.accuracy_score(np.array(real_tags), np.array(predicted_tags))
    return f1_score, acc, predicted_intents, predicted_tags, sentence_correct


print('.....Evaluation....')
f1_score, acc, predicted_intents, predicted_tags, sentence_correct = get_results(data_input_ids, data_input_mask, data_segment_ids, data_valid_positions, data_sequence_lengths,
...
sentence_correct=sentence_correct*1.0/893

print('Slot f1 score = %f' % f1_score)
print('Intent accuracy = %f' % acc)
print('Sentence correectness = %f' % sentence_correct)
...
eva_results = {
    ...
    "intent_accuracy" : acc,
    "Sentence correectness" : sentence_correct
}
