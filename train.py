...
atisIntentArray = ['_UNK', '[CLS]','[SEP]','atis_flight', 'atis_airfare', 'atis_airline', 'atis_ground_service', 'atis_quantity', 'atis_city', 'atis_flight#atis_airfare', 
'atis_abbreviation', 'atis_aircraft', 'atis_distance', 'atis_ground_fare', 'atis_capacity', 'atis_flight_time', 'atis_meal', 'atis_aircraft#atis_flight#atis_flight_no',
'atis_flight_no', 'atis_restriction', 'atis_airport', 'atis_airline#atis_flight_no', 'atis_cheapest', 'atis_ground_service#atis_ground_fare' ]
...
with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
    ...
    while True:
        ...
        if data_processor.end == 1:
            ...
            if arg.dataset=='atis':
                if (epochs == best_epoch_num):
                    outFile = open('Atis_Error_Both.txt', 'w')
                    outFile.truncate(0)
                    for i in range(len(test_pred_intent)):
                        if (test_correct_intent[i] != test_pred_intent[i]) and (test_correct_slot[i] != test_pred_slot[i] ):

                            outFile.write('Input at line: ' + str(i + 1) + ' ' + str(test_words[i]))
                            outFile.write('\n')
                            outFile.write( 'G_I: ' + atisIntentArray[ test_correct_intent[i] ])
                            outFile.write('\n')
                            outFile.write( 'P_I: ' + atisIntentArray[ test_pred_intent[i] ])
                            outFile.write('\n')
                            outFile.write('G_S: ' + str( test_correct_slot[i]))
                            outFile.write('\n')
                            outFile.write('P_S: ' + str(test_pred_slot[i]))
                            outFile.write('\n')
                            outFile.write('------------------------------------------------------------------------------------------------------')
                            outFile.write('\n')
                            outFile.write('\n')

                    outFile = open('Atis_Right_Both.txt', 'w')
                    outFile.truncate(0)
                    for i in range(len(test_pred_intent)):
                        if (test_correct_intent[i] == test_pred_intent[i]) and (test_correct_slot[i] == test_pred_slot[i] ):
                            outFile.write('Input at line: ' + str(i + 1) + ' ' + str(test_words[i]))
                    ...
                    outFile = open('Atis_Error_Slot.txt', 'w')
                    outFile.truncate(0)
                    for i in range(len(test_pred_intent)):
                        if (test_correct_intent[i] == test_pred_intent[i]) and (test_correct_slot[i] != test_pred_slot[i] ):
                            outFile.write('Input at line: ' + str(i+ 1) + ' ' + str(test_words[i]))
                    ...
                    outFile = open('Atis_Error_Intent.txt', 'w')
                    outFile.truncate(0)
                    for i in range(len(test_pred_intent)):
                        if (test_correct_intent[i] != test_pred_intent[i]) and (test_correct_slot[i] == test_pred_slot[i] ):
                            outFile.write('Input at line: ' + str(i+ 1) + ' ' + str(test_words[i]))
